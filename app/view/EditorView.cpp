...
void EditorView::initConnection()
{
    ...
    connect(editArea, &QPlainTextEdit::cursorPositionChanged, this, [=] {
        emit cursorLineChanged(cursorLine());
    });
}
...
{
    if (!editArea || !editArea->document()) {
        return 1;
    }

    return editArea->textCursor().blockNumber() + 1;
}
